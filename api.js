...
let url = require('url'),
	...
	Room = require('./utility/room.js'),
	Question = require('./utility/question.js'),
	Program = require('./utility/program.js');
module.exports = o(function*(req, res, user, post) {
	...
	if (req.url.pathname == '/logout') {
		...
		j = user.notifs.length;
		while (j--) user.notifs[j].unread = false;
		...
		let room = yield Room.getByReferer(req, yield);
		...
		dbcs.chatrooms.update({_id: room._id}, {$push: {invited: invUser.name}});
		res.writeHead(200);
		res.end(JSON.stringify({
			pic: invUser.pic,
			rep: invUser.rep
		...
		dbcs.chatrooms.update({_id: room._id}, {$pull: {invited: post.user}});
		res.writeHead(204);
		res.end();
		...
		dbcs.chat.update({_id: msg._id}, changes);
		res.writeHead(204);
		res.end();
		...
		let tags = post.tags.split(),
			j = tags.length;
		while (j--) {
			if (!(tags[j] = parseInt(tags[j]))) return res.writeHead(400) || res.end('Error: Invalid tag list.');
		}
		...
		dbcs.questions.insert({
			...
			qquestion: post.qquestion.substr(0, 288),
			code: post.code,
			type: post.type,
			tags,
			gr: post.gr == 'on',
